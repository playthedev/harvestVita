      ...
      <section className="bg-[#3A1A3D] pt-44 pb-20 md:pt-52 md:pb-24 relative overflow-hidden">
        ...
        <div className="relative max-w-[1800px] mx-auto px-[5.128vw]">
          <div className="flex items-center gap-3 mb-6">
            ...
            <p className="font-sans-harvest text-[10px] tracking-[0.35em] uppercase text-[#C9A84C]">{returns.eyebrow}</p>
          </div>
          <h1 className="font-display font-bold text-[#F5F0E8] leading-[0.95] tracking-tight mb-4" style={{ fontSize: 'clamp(2.4rem, 5.5vw, 4.5rem)' }}>
            {returns.titleLine1}<br />{returns.titleLine2}
          </h1>
      ...
      <section className="bg-[#F5F0E8] py-20 md:py-28">
        <div className="max-w-[900px] mx-auto px-[5.128vw]">
          ...
          <div className="mt-16 pt-10 border-t border-[#1C1C1C]/10 flex flex-col sm:flex-row gap-4">
            <Link href={localizedHref('/terms', locale)} className="font-sans-harvest text-[10px] tracking-[0.2em] uppercase text-[#C9A84C] hover:text-[#1C1C1C] transition-colors">{returns.footerLinks.terms}</Link>
            <Link href={localizedHref('/privacy', locale)} className="font-sans-harvest text-[10px] tracking-[0.2em] uppercase text-[#C9A84C] hover:text-[#1C1C1C] transition-colors">{returns.footerLinks.privacy}</Link>
          </div>
        </div>
      </section>
    </>
